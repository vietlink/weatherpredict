...
public class WeatherPredict {
    ...
    public static void init(){
        ...
        else if(option==2) initByFile(dataPath);
        isFinished=false;
    }
    ...
    public static void initByFile(String dataPath) {               
        try{
            ...
            for (int i=1;i<X-1; i++){
                for(int j=1; j<Y-1; j++){
                    ...
                    if(k==0){
                        ...
                        W1= ((Wt[i+1][j][k]+Wt[i][j][k])+(Ws[i][j]+Ws[i+1][j]))/4;
                        W2=((Wt[i][j+1][k]+Wt[i][j][k])+(Ws[i][j+1]+Ws[i][j]))/4;
                        //**1
                        ...
                        PS_t2[i][j]=-2*M1*(Ut1[i+1][j][k]*(PS_t1[i+1][j]-PS_t1[i][j])
                                ...
                                -Vt1[i][j][k]*(PS_t1[i][j]-PS_t1[i][j-1]));
                     ...
                     if(k==K-1){
                        ...
                        //***2
                        Vt2[i][j][k]= Vt2[i][j][k]
                                -((Wt[i][j+1][k-1]+Wt[i][j][k-1])*P5*(Vt1[i][j][k]-Vt1[i][j][k-1]))/(PN[k]-PN[k-1]);
    ...
    public static void ChuanDoan(){
                ...
                for(int j=1; j<Y-1; j++){
                    ...
                    if(k==0){
                        ...
                        V_TB=-M*(((Ut1[i+1][j][k]-Ut1[i][j][k])
                            ...
                            +(Vt1[i][j+1][k+1]-Vt1[i][j][k+1]))*P1)/(dex*(P1+P2));                        
                        float PA3= PN[k+1]-P3;
                        float PA2= PN[k]-PN[k+1];
                        float PA1=PN[k]-P3;                        
                        //**1
                        Wt[i][j][k]=(-V_TB*PA1+(Ws[i][j]*PA3/PA2-Wt[i][j][k+1]*PA2/PA3))/(PA3/PA2-PA2/PA3);
                        //**2
                        PHI[i][j][k]=(float) (PHI[i][j][k+1]+R/Ro*TV*Math.pow((PN[k]/P0), RC)*(-PA2));
                    } else if (k==K-1){
                        float PA0= PN[k]-PN[k-1];
                        //***1
                        PHI[i][j][k]= (float) (PHI[i][j][k-1]+R/Ro*TV*Math.pow(PN[k]/P0, RC)*PA0);
                        Wt[i][j][k]=0;
                    }else{
                        float PP1= PN[k-1]-PN[k];
                        float PP2= PN[k-1]-PN[k];
                        float PP3= PN[k]-PN[k+1];
                        PHI[i][j][k]=(float) ((R/Ro*TV*Math.pow(PN[k]/P0, RC)*PP1+PHI[i][j][k-1]*PP3/PP2
        ...
        for (int i = 1; i < X-1; i++) {
            for (int j = 1; j < Y-1; j++) {
                float VTS= M*((Ut1[i+1][j][1]-Ut1[i][j][1])+(Vt1[i][j+1][1]-Vt1[i][j][1]))/dex;
        ...
        for (int k = 0; k < K; k++) {
            for (int i = 0; i < X; i++) {
                for (int j = 0; j < Y; j++) {
                    ...
                    System.out.println((i+1)+"\t"+(j+1)+"\t"+(k+1)+"\t"+Ut2[i][j][k]);
                    System.out.println("V=");
                    System.out.println((i+1)+"\t"+(j+1)+"\t"+(k+1)+"\t"+Vt2[i][j][k]);
                    System.out.println("TE=");
                    System.out.println((i+1)+"\t"+(j+1)+"\t"+(k+1)+"\t"+TE_t2[i][j][k]);
                    System.out.println("Q=");
                    System.out.println((i+1)+"\t"+(j+1)+"\t"+(k+1)+"\t"+Q_t2[i][j][k]);
                }
            }
        }       
    }
