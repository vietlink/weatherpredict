...
//        printResult();
    ...
    public static void DuDoan(){
        ...
        for (int k = 0; k < K; k++) {
            for (int i = 0; i < X; i++) {
                for (int j = 0; j < Y; j++) {
                    System.out.println("Ut2["+k+"]["+i+"]["+j+"]= "+Ut2[k][i][j]);
                }
            }
        }
...
//                    Ro=PN[k]/(R*R0);
                    ...
                    V_TB=-M*((Ut1[k][i+1][j]-Ut1[k][i][j])
                            ...
                            +(Ut1[k+1][i+1][j]-Ut1[k+1][i][j])
                            +(Vt1[k+1][i][j+1]-Vt1[k+1][i][j]))/(dex*2);
                    RW[k]=-V_TB;
                    RTV[k]=(float) ((R/PN[k])*TV*Math.pow(PN[k]/P0, RC)*PP1[k]);
                }
                TV=(float) (TE_t1[K-1][i][j]*(1+0.61*Q_t1[K-1][i][j]));
                PP1[K-1]=PN[K-1]-PN[K-2];
                RTV[K-1]=(float) ((R/PN[K-1])*TV*Math.pow(PN[K-1]/P0, RC)*PP1[K-1]);
